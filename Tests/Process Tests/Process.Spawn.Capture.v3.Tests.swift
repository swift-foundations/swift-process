...
struct ProcessSpawnV3Tests {
    ...
    func concurrentDrainStderr256KiB() throws {
        ...
        #expect(stderrBytes.count == 256 * 1024,
    ...
    func concurrentDrainStdout256KiB() throws {
        ...
        let stdoutBytes = try #require(output.stdout)
        let stderrBytes = try #require(output.stderr)
        #expect(stdoutBytes.count >= 256 * 1024,
    ...
    func concurrentDrainBothPipes128KiB() throws {
        ...
        let stdoutBytes = try #require(output.stdout)
        let stderrBytes = try #require(output.stderr)
        #expect(stdoutBytes.count == 128 * 1024,
                "expected exactly 128 KiB on stdout, got \(stdoutBytes.count)")
        #expect(stderrBytes.count == 128 * 1024,
    ...
    /// test deadline.
    @Test("timeout fires: sleep 30 with 1s timeout → .signaled(SIGKILL)",
          .timeLimit(.minutes(1)))
    ...
    @Test("timeout does not fire when child is fast",
          .timeLimit(.minutes(1)))
    ...
    @Test("timeout fires while pipes are armed",
          .timeLimit(.minutes(1)))
    ...
    @Test("partial capture survives the timeout kill",
          .timeLimit(.minutes(1)))
    func partialCaptureBeforeKill() throws {
        ...
        #expect(text.contains("HELLO_BEFORE_KILL"),
